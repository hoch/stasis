 ...
 *      \/            \/     \/        \/
 *
 * @description   Light-weight static site generator for node.js
...
//   var config_filepath = 'stasis.json';

//   try {
//     // loading stasis.json config file
//     var config = JSON.parse(fs.readFileSync(config_filepath, 'utf8'));
//     return config;
//   } catch (error) {
...
var Core = (function () {
  ...
  // NOT IMPLEMENTED
  function _calculateSimilarity(doc1, doc2) {
    // var distance = 0.0,
    //     keys = Object.keys(doc1.fvec),
    //     keys2 = Object.keys(doc2.fvec);
    // // merge two keys
    // for (var i = 0; i < keys2.length; i++) {
    //   if (keys.indexOf(keys2[i]) < 0) {
    //     keys.push(keys2[i]);
    //   }
    // }
    // for (i = 0; i < keys.length; i++) {
    //   var val1 = (doc1.fvec[keys[i]] || 0);
    //   var val2 = (doc2.fvec[keys[i]] || 0);
    //   distance += Math.pow(val1 - val2, 2);
    // }
    // return Math.round(10000 / distance);
    return 1.0;
  }
